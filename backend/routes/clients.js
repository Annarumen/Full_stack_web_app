...
router.get('/', async (req, res) => {
  try {
    ...
    console.log("Get all clients api call", cli);
    res.json(cli);
  } catch (err) {
...
router.post('/', (req, res, next) => {
  console.log("Call new client api ")
...
router.delete("/:id", (req, res, next) => {
  console.log("Delete client api call");
  clients.findOne({ _id: req.params.id, orgs: org }, (error, data) => {
    if (error) {
      return next(error);
    } else if (!data) {
      ...
      events.find({ attendees: req.params.id, org: org }, (error, eventData) => {
        if (error) {
          return next(error);
        } else {
          // Only delete the client if they are not signed up for any event
          if (eventData.length === 0) {
            clients.findByIdAndDelete(req.params.id, (deleteError, deleteData) => {
              if (deleteError) {
