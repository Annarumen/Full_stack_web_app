  ...
  <main>
    ...
    <div class="px-10 py-20">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
        ...
        <div class="flex justify-between mt-10 mr-20">
          <button @click="showDialog = true" type="submit" class="bg-red-700 disabled:opacity-50 text-white rounded" :disabled="user.role === 'viewer'">
            Delete Client
          </button>
        </div>
        <!--Go back button-->
...
export default {
  ...
  data() {
    return {
      ...
      eventsSelected: [],
      //variable to keep track of whether the dialog should be visible or not.
      showDialog: false,
  ...
  methods: {
    ...
    console.log('hit delete button');
      try {
        console.log("client id ", this.$route.params.id);
      ...
      // Close the dialog after handling the delete action
      this.showDialog = false;
    }
  }
}
